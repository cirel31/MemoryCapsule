...
export default function Navbar() {
  ...
  const [email, setEmail] = useState('jimmy@raynersraiders.com')
  ...
  useEffect(() => {
    const loginCheck = !!JSON.parse(sessionStorage.getItem("loginData"))?.accessToken
    console.log(isLoggedIn)
    setIsLoggedIn(loginCheck)
    if (isLoggedIn) {
      const user_nickname = JSON.parse(sessionStorage.getItem("userInfo"))?.nickname || 'james'
      const user_email = JSON.parse(sessionStorage.getItem("userInfo"))?.email || 'jimmy@raynersraiders.com'
      const user_point = JSON.parse(sessionStorage.getItem("userInfo"))?.point || 1000
      const user_img = JSON.parse(sessionStorage.getItem("userInfo"))?.file || profile_img
          ...
          <nav className="nav_bar">
            ...
            <div className="nav_bar2">
              ...
              부분에 프로필사진, 프로필사진이 없을 경우 디폴트 이미지는 현재 지정된 사진으로*/}
                  <img src={img ? img:profile_img } className="profile_img"/>
                </div>

                {/*USER ID 있는 부분 닉네임 뜨게*/}
                <p className="profile_id">{nickname}</p>
                ...
                <div className="point_box">
                  ...
                  {/*이 아래껀 포인트 떠야함*/}
                  <p className="point_box_txt2">{point}</p>
                </div>
                {/*<div>*/}
                {/*  <Link to='/login'>*/}
                {/*    <button>*/}
